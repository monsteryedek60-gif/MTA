...
--  triggerEvent( 'hud:blur', resourceRoot, 6, false, 0.5, nil )
	...
	local windowWidth, windowHeight = 250, 150
	local bHeight = 35
	...
	showCursor(true)
	wOptions = guiCreateWindow(left, top, windowWidth, windowHeight, "", false)
	...
	if getResourceFromName("admins") and exports['admins']:canPlayerAccessStaffManager(localPlayer) then
	...
	if getResourceFromName("interior_system") and getResourceFromName("interiormanager") and exports.integration:isPlayerAdmin( localPlayer ) then
	...
	bLogout = guiCreateButton(margin, wHeight, 230, 30, "Yeniden Bağlan", false, wOptions)
...
function overlayDescSettings(button, state)
	if source == bOverlayDescription then
		if wOverlayDescSettings then
			...
			guiCreateStaticImage ( 10, 25+y+23, windowWidth-20 , 1, ":admins/images/whitedot.jpg", false, wOverlayDescSettings )
			y = y + 30
			...
			guiCreateStaticImage ( 10, 25+y+21, windowWidth-20 , 1, ":admins/images/whitedot.jpg", false, wOverlayDescSettings )
