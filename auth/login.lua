...
local loginpanelboxok = {
	["login"] = {
		{name = "username", displaytext = "Kullanıcı adı", icon = "user.png", secret = false, maxlen = 30, text = "", input = true},
		{name = "password", displaytext = "Şifre", icon = "pw.png", secret = true, showing = false, maxlen = 30, text = "", input = true},
		{name = "savepw", displaytext = "Beni hatırla", onoff = true, tick = true, animation = {getTickCount()}},
		{name = "loginbutton", displaytext = "Giriş yap", icon = "login.png", button = true},
	},

	["register"] = {
		{name = "username", displaytext = "Kullanıcı adı", icon = "user.png", secret = false, maxlen = 30, text = "", input = true},
		{name = "password", displaytext = "Şifre", icon = "pw.png", secret = true, showing = false, maxlen = 30, text = "", input = true},
		{name = "password2", displaytext = "Şifre x2", icon = "pw.png", secret = true, showing = false, maxlen = 30, text = "", input = true},
		{name = "email", displaytext = "E-mail", icon = "mail.png", secret = false, maxlen = 30, text = "", input = true},
		{name = "invitecode", displaytext = "Referans kodu", icon = "invite.png", secret = false, maxlen = 30, text = "", input = true},
		{name = "registerbutton", displaytext = "Kayıt ol", icon = "reg.png", button = true},
	},

	["char_create"] = {
		{name = "charname", displaytext = "Karakter adı", icon = "name.png", secret = false, maxlen = 30, text = "", input = true},
		{name = "borncity", displaytext = "Doğum yeri", icon = "borncity.png", secret = false, maxlen = 30, text = "", input = true},
		{name = "age", displaytext = "Yaş", icon = "age.png", secret = false, maxlen = 30, text = "", input = true, suffix = "yaşında"},
		{name = "weight", displaytext = "Kilo", icon = "weight.png", secret = false, maxlen = 30, text = "", input = true, suffix = "kilogram"},
		{name = "height", displaytext = "Boy", icon = "height.png", secret = false, maxlen = 30, text = "", input = true, suffix = "boyunda"},
	}
}
...
function renderLoginV2()
	...
	if not renderRememberPanel and not emailChangeShowing then 
		...
		dxDrawText("HOLLY ROLEPLAY", sx*0.435, startY, sx*0.6, startY, tocolor(loginColors[1], loginColors[2], loginColors[3], 240), 1, fontscript:getFont("p_ba", 17/myX*sx), "left", "center")
		...
		dxDrawImage(sx*0.4, startY - sy*0.02, 50/myX*sx, 50/myX*sx,"loginFiles/logo2.png",0,0,0,tocolor(255,255,255,255*pA))
		dxDrawImage(sx*0.4, startY - sy*0.02, 50/myX*sx, 50/myX*sx,"loginFiles/logo1.png",0,0,0,tocolor(loginColors[1], loginColors[2], loginColors[3],255*logoAlpha))

		-- dxDrawImage(sx*0.4, startY - sy*0.02, 50/myX*sx, 50/myX*sx, "loginFiles/logo.png", 0, 0, 0, tocolor(loginColors[1], loginColors[2], loginColors[3], 240))
		...
		if loginPanelPage == "login" then 	
			dxDrawText("Giriş yap", sx*0.435, startY, sx*0.6, startY, tocolor(255, 255, 255, 240), 1, fontscript:getFont("p_m", 13/myX*sx), "left", "center")
			startY = startY + sy*0.02
		elseif loginPanelPage == "register" then 
			dxDrawText("Kayıt ol", sx*0.435, startY, sx*0.6, startY, tocolor(255, 255, 255, 240), 1, fontscript:getFont("p_m", 13/myX*sx), "left", "center")
			startY = startY + sy*0.02
		end
		...
		for k, v in ipairs(loginpanelboxok[loginPanelPage]) do 
			...
			if v.input then
				...
				dxDrawImage(sx*0.4 + iconMargin/myX*sx, startY + iconMargin/myY*sy, barHeight - (iconMargin*2)/myY*sy, barHeight - (iconMargin*2)/myY*sy, "loginFiles/v2_icons/"..v.icon, 0, 0, 0, tocolor(255, 255, 255, displayAlpha))
				...
				if v.secret then 
					if core:isInSlot(sx*0.6 - barHeight + (iconMargin * 0.9)/myY*sy, startY + (iconMargin * 0.9)/myY*sy, barHeight - ((iconMargin * 0.9)*2)/myY*sy, barHeight - ((iconMargin * 0.9)*2)/myY*sy) then 
						dxDrawImage(sx*0.6 - barHeight + (iconMargin * 0.9)/myY*sy, startY + (iconMargin * 0.9)/myY*sy, barHeight - ((iconMargin * 0.9)*2)/myY*sy, barHeight - ((iconMargin * 0.9)*2)/myY*sy, "loginFiles/v2_icons/toggle.png", 0, 0, 0, tocolor(255, 255, 255, 140))
					else
						dxDrawImage(sx*0.6 - barHeight + (iconMargin * 0.9)/myY*sy, startY + (iconMargin * 0.9)/myY*sy, barHeight - ((iconMargin * 0.9)*2)/myY*sy, barHeight - ((iconMargin * 0.9)*2)/myY*sy, "loginFiles/v2_icons/toggle.png", 0, 0, 0, tocolor(255, 255, 255, 100))
					end
				end
			elseif v.button then 
				...
				dxDrawImage((sx*0.4 + sx*0.2 / 2) - 10/myX * sx + dxGetTextWidth(v.displaytext,1, fontscript:getFont("p_bo", 12/myX*sx))/2, startY + barHeight / 2 - 10/myX*sx, 20/myX*sx, 20/myX*sx, "loginFiles/v2_icons/"..v.icon, 0, 0, 0, tocolor(255, 255, 255, 255))
			...
			elseif v.onoff then 
				...
				if savedatas == true then 
					...
					dxDrawImage(sx*0.4, startY, sy*0.025, sy*0.025, "loginFiles/v2_icons/check.png", 0, 0, 0, tocolor(loginColors[1], loginColors[2], loginColors[3], 255 * animalpha))
				end
			end
		...
		if loginPanelPage == "login" then 
			dxDrawText("Henüz bir hesabın yok mu?", sx*0.4, startY + sy*0.02, sx*0.6, startY + sy*0.02, tocolor(255, 255, 255, 100), 1, fontscript:getFont("p_r", 12/myX*sx), "center", "center")

			if core:isInSlot(sx*0.4, startY + sy*0.03, sx*0.2, sy*0.01) then 
				dxDrawText("Kayıt ol", sx*0.4, startY + sy*0.035, sx*0.6, startY + sy*0.035, tocolor(loginColors[1], loginColors[2], loginColors[3], 240), 1, fontscript:getFont("p_m", 12/myX*sx), "center", "center")
			else
				dxDrawText("Kayıt ol", sx*0.4, startY + sy*0.035, sx*0.6, startY + sy*0.035, tocolor(loginColors[1], loginColors[2], loginColors[3], 200), 1, fontscript:getFont("p_m", 12/myX*sx), "center", "center")
			end
		...
		elseif loginPanelPage == "register" then 
			dxDrawText("Zaten bir hesabın var mı?", sx*0.4, startY + sy*0.02, sx*0.6, startY + sy*0.02, tocolor(255, 255, 255, 100), 1, fontscript:getFont("p_r", 12/myX*sx), "center", "center")

			if core:isInSlot(sx*0.4, startY + sy*0.03, sx*0.2, sy*0.01) then 
				dxDrawText("Giriş yap", sx*0.4, startY + sy*0.035, sx*0.6, startY + sy*0.035, tocolor(loginColors[1], loginColors[2], loginColors[3], 240), 1, fontscript:getFont("p_m", 12/myX*sx), "center", "center")
			else
				dxDrawText("Giriş yap", sx*0.4, startY + sy*0.035, sx*0.6, startY + sy*0.035, tocolor(loginColors[1], loginColors[2], loginColors[3], 200), 1, fontscript:getFont("p_m", 12/myX*sx), "center", "center")
			end
		end
	...
	if isInBox(30/myX*sx,sy*0.97, 15/myX*sx, 15/myX*sx) then 
		if soundState then 
			dxDrawImage(30/myX*sx,sy*0.97, 15/myX*sx, 15/myX*sx,"loginFiles/pause.png",0,0,0,tocolor(255,255,255,255))
		else
			dxDrawImage(30/myX*sx,sy*0.97, 15/myX*sx, 15/myX*sx,"loginFiles/play.png",0,0,0,tocolor(255,255,255,255))
		end
	else
		if soundState then
			dxDrawImage(30/myX*sx,sy*0.97, 15/myX*sx, 15/myX*sx,"loginFiles/pause.png",0,0,0,tocolor(255,255,255,100))
		else
			dxDrawImage(30/myX*sx,sy*0.97, 15/myX*sx, 15/myX*sx,"loginFiles/play.png",0,0,0,tocolor(255,255,255,100))
		end
	end

	if isInBox(50/myX*sx,sy*0.97,15/myX*sx, 15/myX*sx) then 
		dxDrawImage(50/myX*sx,sy*0.97,15/myX*sx, 15/myX*sx,"loginFiles/next-track.png",0,0,0,tocolor(255,255,255,255))
	else
		dxDrawImage(50/myX*sx,sy*0.97,15/myX*sx, 15/myX*sx,"loginFiles/next-track.png",0,0,0,tocolor(255,255,255,100))
	end

	if isInBox(10/myX*sx,sy*0.97,15/myX*sx, 15/myX*sx) then 
		dxDrawImage(10/myX*sx,sy*0.97,15/myX*sx, 15/myX*sx,"loginFiles/next-track.png",180,0,0,tocolor(255,255,255,255))
	else
		dxDrawImage(10/myX*sx,sy*0.97,15/myX*sx, 15/myX*sx,"loginFiles/next-track.png",180,0,0,tocolor(255,255,255,100))
	end

	dxDrawImage(80/myX*sx,sy*0.97,15/myX*sx, 15/myX*sx,"loginFiles/v2_icons/volume.png",0,0,0,tocolor(255,255,255,100))
							...
							if v.name == "loginbutton" then 
								if mehetaclick then
									...
									if user and pass then
										...
										triggerServerEvent('accounts:login:attempt', localPlayer, user, pass, false)
							...
							elseif v.name == "registerbutton" then 
								if mehetaclick then
									...
									if user and pass and string.len(user) >= 3 and string.len(pass) >= 5 and string.len(pass2) >= 5 and string.len(email) >= 5 then
										if pass == pass2 then 
											triggerServerEvent('referenceEvent', resourceRoot, localPlayer, inviteCode)
											triggerServerEvent('accounts:register:attempt', localPlayer, user, pass, email)
...
function renderCharCreate()
	...
	dxDrawText("HOLLY ROLEPLAY", sx*0.135, sy*0.2, sx*0.6, sy*0.2, tocolor(loginColors[1], loginColors[2], loginColors[3], 240), 1, fontscript:getFont("p_ba", 17/myX*sx), "left", "center")
	dxDrawImage(sx*0.1, sy*0.2 - sy*0.02, 50/myX*sx, 50/myX*sx, "loginFiles/logo.png", 0, 0, 0, tocolor(loginColors[1], loginColors[2], loginColors[3], 240))
	dxDrawText("Karakter oluşturma", sx*0.135, sy*0.22, sx*0.6, startY, tocolor(255, 255, 255, 240), 1, fontscript:getFont("p_m", 13/myX*sx), "left", "center")
	...
	for k, v in ipairs(loginpanelboxok["char_create"]) do 
		...
		if v.input then
			...
			dxDrawImage(sx*0.1 + iconMargin/myX*sx, startY + iconMargin/myY*sy, barHeight - (iconMargin*2)/myY*sy, barHeight - (iconMargin*2)/myY*sy, "loginFiles/v2_icons/"..v.icon, 0, 0, 0, tocolor(255, 255, 255, displayAlpha))
		end
	...
	startY = startY + sy*0.02
	dxDrawText("Cinsiyetinizi ve kıyafetinizi seçin:", sx*0.1, startY, sx*0.6, startY, tocolor(255, 255, 255, 255), 1, fontscript:getFont("p_m", 13/myX*sx), "left", "center")
	startY = startY + sy*0.02

	if selected_nem == 1 or core:isInSlot(sx*0.1, startY, 40/myX*sx, 40/myX*sx) then 
		dxDrawImage(sx*0.1, startY, 40/myX*sx, 40/myX*sx, "loginFiles/no.png", 0, 0, 0, tocolor(loginColors[1], loginColors[2], loginColors[3], 240))
	else
		dxDrawImage(sx*0.1, startY, 40/myX*sx, 40/myX*sx, "loginFiles/no.png", 0, 0, 0, tocolor(255, 255, 255, 150))
	end

	if selected_nem == 0 or core:isInSlot(sx*0.1 + 45/myX*sx, startY, 40/myX*sx, 40/myX*sx) then
		dxDrawImage(sx*0.1 + 45/myX*sx, startY, 40/myX*sx, 40/myX*sx, "loginFiles/ferfi.png", 0, 0, 0, tocolor(loginColors[1], loginColors[2], loginColors[3], 240))
	else
		dxDrawImage(sx*0.1 + 45/myX*sx, startY, 40/myX*sx, 40/myX*sx, "loginFiles/ferfi.png", 0, 0, 0, tocolor(255, 255, 255, 150))
	end

	if core:isInSlot(sx*0.3 - 40/myX*sx, startY, 40/myX*sx, 40/myX*sx) then 
		dxDrawImage(sx*0.3 - 40/myX*sx, startY, 40/myX*sx, 40/myX*sx, "loginFiles/arrow.png", 0, 0, 0, tocolor(loginColors[1], loginColors[2], loginColors[3], 240))
	else
		dxDrawImage(sx*0.3 - 40/myX*sx, startY, 40/myX*sx, 40/myX*sx, "loginFiles/arrow.png", 0, 0, 0, tocolor(255, 255, 255, 150))
	end

	if core:isInSlot(sx*0.3 - 85/myX*sx, startY, 40/myX*sx, 40/myX*sx) then 
		dxDrawImage(sx*0.3 - 85/myX*sx, startY, 40/myX*sx, 40/myX*sx, "loginFiles/arrow.png", -180, 0, 0, tocolor(loginColors[1], loginColors[2], loginColors[3], 240))
	else
		dxDrawImage(sx*0.3 - 85/myX*sx, startY, 40/myX*sx, 40/myX*sx, "loginFiles/arrow.png", -180, 0, 0, tocolor(255, 255, 255, 150))
	end

	startY = startY + sy*0.07
	dxDrawText("Avatar seçin:", sx*0.1, startY, sx*0.6, startY, tocolor(255, 255, 255, 255), 1, fontscript:getFont("p_m", 13/myX*sx), "left", "center")
	startY = startY + sy*0.02
	...
	dxDrawText("Başlangıç bölgesi:", sx*0.1, startY, sx*0.6, startY, tocolor(255, 255, 255, 255), 1, fontscript:getFont("p_m", 13/myX*sx), "left", "center")
	...
	if pendingCharCreate then 
		...
		dxDrawImage(((sx*0.1 + sx*0.2 / 2) - 10/myX * sx ), startY + barHeight / 2 - 10/myX*sx, 20/myX*sx, 20/myX*sx, "loginFiles/v2_icons/loading.png", spinnerRot, 0, 0, tocolor(255, 255, 255, 255))
	else
		dxDrawText("Karakter oluştur", sx*0.1, startY, sx*0.3 - 30/myX*sx, startY + barHeight + sy*0.004, tocolor(255, 255, 255, 255), 1, fontscript:getFont("p_bo", 13/myX*sx), "center", "center")
		dxDrawImage((sx*0.1 + sx*0.2 / 2) - 10/myX * sx + dxGetTextWidth("Karakter oluştur",1, fontscript:getFont("p_bo", 13/myX*sx))/2, startY + barHeight / 2 - 10/myX*sx, 20/myX*sx, 20/myX*sx, "loginFiles/v2_icons/reg.png", 0, 0, 0, tocolor(255, 255, 255, 255))
	end
end 
									...
									if age and (age >= 18 and age <= 99) then 
										if weight and (weight >= 50 and weight <= 200) then 
											if height and (height >= 150 and height <= 240) then 
												...
												triggerServerEvent("accounts:characters:new", localPlayer, characterName, "yarak", 1, selected_nem, skin, height, weight, age, 1, 1, 1, selectedStartPos)
...
function charLoadAnimation()
	...
	dxDrawImage(0, 0, sx, sy, "load.jpg", 0, 0, 0, tocolor(255, 255, 255, 255 * loadPanelAlpha))
	dxDrawRectangle(0,sy*0.99,sx*load,sy,tocolor(255,255,255,255*loadPanelAlpha))
	dxDrawText(math.floor(load*100).."%",0,sy*0.99,0+sx*load,sy*0.99+sy*0.0125,tocolor(0, 0, 0,255*loadPanelAlpha),0.65/myX*sx,"default-bold","center","center")
	...
	dxDrawText("hollyRoleplay", 0, sy*0.15, sx, sy*0.15, tocolor(255, 255, 255, 100 * loadPanelAlpha), 1, fontscript:getFont("p_ba", 14/myX*sx), "center", "center")
	dxDrawText("Aramıza hoş geldin!", 0, sy*0.18, sx, sy*0.18, tocolor(255, 255, 255, 255 * loadPanelAlpha), 1, fontscript:getFont("p_ba", 30/myX*sx), "center", "center")
	dxDrawText("Karakter yükleniyor...", 0, sy*0.21, sx, sy*0.21, tocolor(255, 255, 255, 150 * loadPanelAlpha), 1, fontscript:getFont("p_bo", 14/myX*sx), "center", "center")

	dxDrawImage(sx/2-200/myX*sx + 7/myX*sx,sy/2-200/myX*sx,400/myX*sx, 400/myX*sx,"loginFiles/logo.png",0,0,0,tocolor(255,255,255,30*loadPanelAlpha))
	local skinId = getElementModel(localPlayer)
	dxDrawImage(sx/2-100/myX*sx,sy/2 - 496/2/myY*sy,200/myX*sx,496/myY*sy,"loginFiles/peds/"..skinId..".png",0,0,0,tocolor(255,255,255,100*loadPanelAlpha))
	dxDrawImageSection(sx/2-100/myX*sx,(sy/2 - 496/2/myY*sy),200/myX*sx,496/myY*sy * load, 0, 0, 200,496 * load,"loginFiles/peds/"..skinId..".png",0, 0,0,tocolor(255,255,255,255*loadPanelAlpha))

	--dxDrawImage(0,0,sx,sy,"loginFiles/logo1.png",0,0,0,tocolor(255,255,255,255*logoAlpha*loadPanelAlpha))

	local greatingText = "Bir sorun ile mi karşılaştın? Yardıma mı ihtiyacın var? Discord üzerinden bizlere ulaşabilirsin!"
...
function renderBanPanel() 
	...
	--dxDrawImage(0,0,sx,sy,"loginFiles/logo2.png",0,0,0,tocolor(255,255,255,255*pA))

	--dxDrawImage(0,0,sx,sy,"loginFiles/logo1.png",0,0,0,tocolor(255,255,255,255*logoAlpha))
...
function newCharacter_response(statusID, statusSubID)
	if (statusID == 1) then
		outputChatBox("Bilinmeyen bir hata oluştu! Ne0R` ile iletişime geçin. \nHata Kodu : ACC#"..tostring(statusSubID) )
	elseif (statusID == 2) then
		if (statusSubID == 1) then
			outputChatBox( "Böyle bir isim zaten kullanılmaktadır." )
		else
			outputChatBox( "Bilinmeyen bir hata oluştu! Ne0R` ile iletişime geçin. \nHata Kodu : ACD#"..tostring(statusSubID) )
		end
